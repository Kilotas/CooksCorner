...
class LikeAPIView(APIView):
    ...
    def post(self, request, recipe_id):
        recipe = Recipe.objects.get(pk=recipe_id) # поучае объект Recipe по его идентификатору
        like, created = Like.objects.get_or_create(users=request.user, recipe=recipe) # Пытаемся получить объект Like для данного пользователя и рецепта, или создаем новый, если он не существует.
        if not created:
